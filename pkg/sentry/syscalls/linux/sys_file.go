...
func CloseRange(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	if cloexec != 0 {
		...
		for fd := first; fd < last; fd++ {
			...
			t.FDTable().SetFlags(t.AsyncContext(), int32(fd), flagToApply)
		}
	} else {
		for fd := first; fd <= last; fd++ {
			file, _ := t.FDTable().Remove(t, int32(fd))
			if file == nil {
				continue
			}
			defer file.DecRef(t)
			...
			err := file.Flush(t)
			handleIOError(t, false /* partial */, err, linuxerr.EINTR, "close_range", file)
		}
	}
